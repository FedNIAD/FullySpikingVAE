...
def test(network, testloader):
    ...
    with torch.no_grad():
        ...
        for batch_idx, (real_img, labels) in enumerate(testloader):
            ...
            print(
                f'Test[{batch_idx}/{len(testloader)}] Loss: {loss_meter.avg}, RECONS: {recons_meter.avg}, DISTANCE: {dist_meter.avg}')
            result.loc[batch_idx] = [labels[0], loss_meter.avg, recons_meter.avg, dist_meter.avg]
            print
        ...
        result.to_csv('result_fedavg.csv')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-checkpoint', action='store', dest='checkpoint', default='checkpoint/FEDAVG/checkpoint.pth',
    ...
    if dataset_name == "MNIST":
        ...
        data_path = os.path.expanduser(data_path)
        train_loader, test_loader = load_dataset_snn.load_mvtec(data_path,0,1,1)
    ...
    #         best_loss = test_loss
    #         torch.save(net.state_dict(), f'checkpoint/{args.name}/best1500.pth')
    #
    # sample(net, 150, batch_size=16)
